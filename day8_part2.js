...
solveButton.addEventListener('click', () => {
  ...
  // halt when all nodes have been connected
  for (let i = 0; maxSize < inputArray.length ; i++) {
    ...
    maxSize = Math.max(maxSize, newParent.rank)

    if (maxSize === inputArray.length) {
      const nodeAx = parseInt(nodeAName.split(',')[0])
      const nodeBx = parseInt(nodeBName.split(',')[0])
      result = nodeAx * nodeBx
    }
  }
